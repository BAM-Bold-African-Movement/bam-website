...
/**
 * Supported token interface for return values
 */
 ...
 * @returns {NetworkConfig|null} The network configuration or null if not supported
 */
 ...
 * @returns {TokenInfo|null} The corresponding CoinGecko token info or null if not found
 */
export const getTokenInfo = (chainId: number, tokenAddress: string | null = null): TokenInfo | null => {
 ...
 */
export const fetchExchangeRate = async (chainId: number, tokenAddress: string | null = null): Promise<number> => {
...
export const convertToTokenAmount = (
  ...
  const tokenAmount = parseFloat(usdAmount.toString()) * exchangeRate;
 ...
 */
export const getTokenSymbol = (chainId: number, tokenAddress: string | null = null): string => {
  const tokenInfo = getTokenInfo(chainId, tokenAddress);
 ...
 */
export const getSupportedTokens = (chainId: number): SupportedToken[] => {
  const network = getNetworkConfig(chainId);
  if (!network) return [];
